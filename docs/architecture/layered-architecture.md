...
3. ApiSignatureMiddleware（写接口）
   └─ 校验 X-App-Key / X-Timestamp / X-Nonce / X-Signature，落库 nonce 防重放
4. ApiIdempotencyMiddleware（创建/状态变更接口）
   └─ 按 tenant_id + Idempotency-Key 复用首次响应，防止重复下单/重复操作
5. ApiRateLimitMiddleware
   └─ Redis SlidingWindow 检查日配额 → 分级策略（basic 硬阻断 / pro 软告警）
6. SqlTenantGuard（注册全局监听）
7. OrderController → OrderQueryService → Order Model (TenantScope 自动过滤)
8. OctaneTenantCleanupMiddleware
...
5. **HMAC 签名 + nonce 防重放** — 敏感写接口防抓包重放和 AppKey 混用
6. **Idempotency-Key 幂等** — 创建订单/状态变更接口复用首次响应，避免重复交易
7. **轻量规则引擎** — 可配置阈值 + 自动告警，无重型规则框架
8. **月结半自动对账** — 系统算应收 + 人工填实报 + 自动差异单
9. **Impersonation 审计链** — 平台→商户上下文切换全程留痕
