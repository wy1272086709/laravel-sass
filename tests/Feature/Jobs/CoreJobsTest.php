...
it('closes expired pending payment orders only', function () {
    ...
    app(CloseExpiredOrderJob::class, ['orderId' => $expired->id])->handle(app(QueueJobLogger::class));
    app(CloseExpiredOrderJob::class, ['orderId' => $paid->id])->handle(app(QueueJobLogger::class));
